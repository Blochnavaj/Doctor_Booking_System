...
function MyProfile() {
  ...
  return (
    <div className='max-w-lg flex flex-col text-sm gap-2'>
      ...
      {
        isEdit ? <input className='bg-gray-50 text-3xl font-medium mt-4 max-w-60 ' type="text" value={userData.name} onChange={e => setUserData(prev => ({ ...prev, name: e.target.value }))} /> : <p className='font-medium text-3xl text-neutral-800 mt-4'>{userData.name}</p>
      } 
      ...
      <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
        ...
        {
          isEdit 
          ? <input className='max-w-28 bg-gray-100 outline-none' type="date" onChange={(e) => setUserData(prev => ({...prev, dob: e.target.value}))}   value={userData.dob}/>
